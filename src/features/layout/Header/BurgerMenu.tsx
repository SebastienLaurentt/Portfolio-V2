...
export default function BurgerMenu({ isOpen, setIsOpen }: IBurgerMenu) {
  ...
  return (
    <>
      ...
      >
        <Menu  className="h-8 md:h-10 w-8 md:w-10" />
      </button>

      <nav
        className={`flex-col min-h-screen top-0 w-full fixed bg-background text-center transition-all duration-500 ease-in-out  ${
          isOpen ? "left-0" : "left-full"
        }`}
      >
        <div className=" px-6 md:px-10 py-6 mb-12">
          <X
